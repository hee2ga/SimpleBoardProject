...
public class CheckField {
	...
	Set<String> nicknameSet=boardService.inputNickname();
	...
	public String checkWriter(String writer) {
		...
		if(nicknameSet.contains(writer)) {
	...
	// 닉네임 존재하는지
	public boolean checkNickname(String writer) {
		return nicknameSet.contains(writer);
	}
	
	// 닉네임과 패스워드 일치검사(닉네임 검사 후 실행)
	public boolean checkPassAndNickname(int bno,String writer,String password) {
		for(Board board:list) {
			if(board.getBno()==bno) {
				return board.getWriter().equals(writer)&&board.getPassword().equals(password);
			}	
		}
